...
try {
  ...
  console.log(
    ...
    Object.keys((protos.user as any)?.user?.UserService?.service || {})
  );
  console.log(
    "Auth Service methods:",
    Object.keys((protos.auth as any)?.auth?.AuthService?.service || {})
  );
  console.log(
    "Notification Service methods:",
    Object.keys(
      (protos.notification as any)?.notification?.NotificationService
        ?.service || {}
    )
  );
} catch (error) {
...
console.log(
  "- Có thể version proto files bằng cách create folders v1/, v2/, etc."
);
